...
class GroupedTaskDataset(Dataset):
    def __init__(self, sequence_datasets, task_embs, cfg):
...
class CustomSequenceVLDataset(SequenceVLDataset):
    def __init__(self, sequence_dataset, task_emb, cfg):
    ...
    def __getitem__(self, idx):
        ...
        for key in return_dict['obs']:
            original_shape = return_dict['obs'][key].shape            
            # 重塑数组，只改变第一个维度（时间步），保持其他维度不变
            reshaped = return_dict['obs'][key].reshape(self.seq_len, -1)
            
            # 只取前self.n_obs_steps步，其他维度保持不变
            return_dict['obs'][key] = reshaped[:self.n_obs_steps].reshape(-1, *original_shape[1:])

        # Modify actions
        return_dict['actions'] = return_dict['actions'][:self.horizon]
...
class TaskDataset(Dataset):
    def __init__(self, features, actions):
        ...
        assert len(features) == len(actions), "Features and actions must have same length"
