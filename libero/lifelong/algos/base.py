...
class Sequential(nn.Module, metaclass=AlgoMeta):
    ...
    def __init__(self, n_tasks, cfg):
        ...
        self.policy = get_policy_class(cfg.policy.policy_type)(cfg, cfg.shape_meta, n_task=self.n_tasks)
        self.current_task = -1
    ...
    def start_task(self, task):
        ...
        if (self.cfg.use_r3m) and (self.current_task == 0):
            all_params = (
                list(self.policy.parameters()) +  # 策略参数
                list(self.encoder.parameters()) 
            )
        else:
            all_params = (self.policy.parameters())
        self.optimizer = eval(self.cfg.train.optimizer.name)(
            all_params, **self.cfg.train.optimizer.kwargs
        )
    ...
    def learn_one_task(self, dataset, task_id, benchmark, result_summary):
        ...
        last_model_checkpoint_name = os.path.join(
            self.experiment_dir, f"task{task_id}_last_model.pth"
        )
        ...
        self.policy.load_state_dict(torch_load_model(last_model_checkpoint_name)[0])
    ...
    def verify_frozen_state(self, model):
        issues = []

        if model.training:
            issues.append("Model is in training mode")
        
        for name, param in model.named_parameters():
            if param.requires_grad:
                issues.append(f"Parameter {name} requires_grad is True")
        

        for name, m in model.named_modules():
            if isinstance(m, nn.BatchNorm2d):
                if m.training:
                    issues.append(f"BatchNorm {name} is in training mode")
                if m.track_running_stats:
                    issues.append(f"BatchNorm {name} is tracking running stats")
        
        if issues:
            print("Found following issues:")
            for issue in issues:
                print(f"- {issue}")
        else:
            print("Model is correctly frozen")