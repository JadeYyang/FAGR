...
from libero.lifelong.datasets import GroupedTaskDataset, SequenceVLDataset, get_dataset, CustomSequenceVLDataset
from libero.lifelong.metric import evaluate_loss, evaluate_success, get_auc, compute_metric
...
def main(hydra_cfg):
    ...
    if gsz == 1:  # each manipulation task is its own lifelong learning task
        if cfg.policy.policy_type in ('BCDiffusionPolicy', 'BCMeanFlowPolicy'):
            datasets = [
                CustomSequenceVLDataset(ds, emb, cfg) for (ds, emb) in zip(manip_datasets, task_embs)
            ]
        else:
            datasets = [
        ...
        for i in range(0, n_manip_tasks, gsz):
            dataset = GroupedTaskDataset(
                manip_datasets[i : i + gsz], task_embs[i : i + gsz], cfg
            )
    ...
    print("[info] computing FWT,BWT,AUC\n")
    fwds = get_auc(cfg.experiment_dir, cfg, N_TASKS=n_manip_tasks, N_SEEDS=1, seeds=[cfg.seed])
